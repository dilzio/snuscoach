...
| Wins & Posts section: wins ledger + post history + AI drafting chat + save post | ✅ | `feature/wins-posts-ui`; meetings-style list→detail UX: wins table (title, description, date, posted ✓ indicator, [↗ New Post]) + posts table (channel, date, audience, win); win detail view with edit form, linked posts list, contextual chat (thread per win, no auto-LLM), Adopt AI draft button, Save Win; 48 Playwright tests |
